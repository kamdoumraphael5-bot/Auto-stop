...
export default function BookingScreen({ route, navigation }) {
  ...
  const translations = {
    fr: {
      ...
      mobileMoneyDesc: 'Orange Money, MTN Mobile Money',
      card: '💳 Carte bancaire',
  ...
  // ============ FONCTION HANDLEBOOKING CORRIGÉE ============
  const handleBooking = async () => {
    ...
    setLoading(true);
    try {
      // Utiliser ride.rideId (l'objet ride a un champ "rideId", pas "id")
      ...
      // Initier le paiement avec le moyen choisi
      const paymentResponse = await fetch(`${API_URL}/api/payment/initiate`, {
        ...
        body: JSON.stringify({
          rideId: rideIdToUse,
          paymentMethod: selectedPaymentType, // 'mobile_money' ou 'card'
      ...
      if (paymentResponse.ok && paymentData.paymentUrl) {
        // Stocker les infos de réservation pour le callback
        const bookingInfo = {
          rideId: rideIdToUse,
          seats: 1,
          bookerName,
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {/* Sélection du moyen de paiement (Mobile Money ou Carte) */}
...
const styles = StyleSheet.create({
  ...
  // Styles pour la sélection du moyen de paiement
